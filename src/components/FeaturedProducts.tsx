  ...
  return (
    <section className="pt-16 sm:pt-20 pb-16 bg-neutral-100" id="models">
      ...
      <div className="container mx-auto px-4 mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-neutral-900">Naš Model</h2>
        <p className="text-base sm:text-lg text-neutral-600 text-center mt-2 max-w-2xl mx-auto">
          ...
          {featuredProducts.map((product) => (
            ...
            >
              ...
              <div className="p-3 sm:p-6 bg-neutral-50">
                <span className="text-[10px] sm:text-sm uppercase tracking-wider text-neutral-500">
                  {product.category}
                </span>
                <h3 className="text-lg sm:text-2xl font-bold mt-1 mb-1 sm:mb-2 text-neutral-900">{product.title}</h3>
                <p className="text-xs sm:text-base text-neutral-600 mb-2 sm:mb-4">{product.description}</p>
                <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-4">
                  <p className="text-base sm:text-xl font-bold text-neutral-900">
                    {product.price.toLocaleString()} RSD
                  </p>
                  <p className="text-sm sm:text-lg text-neutral-500 line-through">
                    {product.originalPrice.toLocaleString()} RSD
                  </p>
                </div>
                <Button 
                  className="w-full bg-neutral-900 text-neutral-50 hover:bg-neutral-800 py-2 sm:py-4 text-xs sm:text-base group-hover:bg-neutral-800"
                >
                  Saznaj Više
                </Button>
              </div>
